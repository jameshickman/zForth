...
# Shared library
%{__cc} $CFLAGS -fPIC -DPIC -c src/zforth/zforth.c -o zforth.shared.o
...
Name: zforth
Description: Small, embeddable Forth interpreter and compiler
URL: %{url}
...
Libs: -L\${libdir} -lzforth -Wl,--export-dynamic
Libs.private: -lm
...
stray=$(nm -D --undefined-only lib%{name}.so.%{libversion} | awk '{print $NF}' | \
        grep '^zf_' | grep -v '^zf_host_' || :)
test -z "$stray" || { echo "unexpected undefined zf_ symbols: $stray" >&2; exit 1; }
...
%{__cc} %{build_cflags} $(pkg-config --cflags zforth) %{SOURCE2} \
        $(pkg-config --libs zforth) -lm -o example-check
test "$(./example-check | tr -d ' \r')" = "49" || \
  { echo "example program did not print 49" >&2; exit 1; }
...
for sym in zf_init zf_bootstrap zf_eval zf_dump zf_abort zf_push zf_pop \
  ...
  nm -D --defined-only lib%{name}.so.%{libversion} | grep -qw "$sym" || \
    { echo "$sym is missing from the library" >&2; exit 1; }
done
